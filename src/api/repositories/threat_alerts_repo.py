...
def get_all(
    ...
    params: list = []
    if status:
        clauses.append("ta.status = ?")
        params.append(status)
    if alert_type:
        clauses.append("ta.alert_type = ?")
        params.append(alert_type)
    if asset_id:
        clauses.append("ta.asset_id = ?")
        params.append(asset_id)
    if is_kev is not None:
        clauses.append("ta.is_kev = ?")
        params.append(int(is_kev))
    ...
    rows = conn.execute(
        f"""SELECT ta.*, a.hostname, a.ip_address, a.business_unit, a.environment
            ...
            WHERE {where}
            ORDER BY ta.is_kev DESC, ta.epss_score DESC, ta.created_at DESC
            LIMIT ? OFFSET ?""",
        params + [limit, offset],
    ).fetchall()
