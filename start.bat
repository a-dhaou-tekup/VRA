...
start "VRA Backend" cmd /c "set PYTHONIOENCODING=utf-8 && set JWT_SECRET_KEY=8bcc5f6893289288bcd65f53e7e28360e9d6eb09946ddd44ea4b9a67bd0d6a22 && set PLATFORM_DB_PATH=data/cache/platform.db && python run_api.py"
timeout /t 3 /nobreak >nul

REM ── 3. Frontend ────────────────────────────────────────────────────────────
echo  [3/3] Starting frontend (http://localhost:5173)...
start "VRA Frontend" cmd /c "cd frontend && npm run dev"
